...
function AdversaryCard({ adversary, partyTier, onAdd, onShowDetails }) {
  ...
  const adjustedBP = adversary.tier < partyTier 
  ...
  return (
    <ListGroup.Item className="d-flex justify-content-between align-items-center bg-dark text-light border-secondary">
      <div onClick={() => onShowDetails(adversary)} style={{ cursor: 'pointer', width: '100%'}}>
        <span className="fw-bold">{adversary.name}</span>
        <div className="text-muted small">
            <Badge pill bg={variant} className="me-2">T{adversary.tier}</Badge>
            {adversary.type}
            <span className="mx-2">|</span>
            HP: {adversary.hp}
            <span className="mx-2">|</span>
            BP: {adjustedBP} {adjustedBP !== adversary.battle_points && `(${adversary.battle_points})`}
        </div>
      </div>
      <Button variant="outline-primary" size="sm" onClick={() => onAdd(adversary)}>Add</Button>
    </ListGroup.Item>
  );
}
